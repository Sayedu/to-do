...
export async function GET() {
    try {
      ...
      // Pretty format the JSON response with 2 spaces of indentation
      const prettyJson = JSON.stringify(tasks, null, 2);
  
      return new Response(prettyJson, { status: 200 });
    } catch (err) {
      return new Response(
        JSON.stringify({ message: 'Error fetching tasks' }, null, 2), 
        { status: 500 }
      );
    }
  }
